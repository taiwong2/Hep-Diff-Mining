...
Steps:
  ...
  16. Readout analysis (rank genes by consistency, dynamic range, coverage)
  17. Generate manuscript figures from current database state
...
def step16_readout_analysis(db) -> None:
    """Rank expression readouts by consistency, dynamic range, and coverage."""
    print("\n" + "=" * 60)
    print("Step 16: Readout Analysis")
    print("=" * 60)

    from scripts.readout_analysis import run as readout_run

    readout_run(db)


def step17_generate_figures(db) -> None:
    """Generate all manuscript figures from current database state."""
    print("\n" + "=" * 60)
    print("Step 17: Generate Figures")
    print("=" * 60)

    fig_dir = PROJECT_ROOT / "manuscript" / "figures"

    from scripts.generate_figures import run as analysis_figures_run
    analysis_figures_run(db, out_dir=fig_dir)

    from scripts.generate_figures_v2 import run as methods_figures_run
    methods_figures_run(db, out_dir=fig_dir)
...
def main() -> None:
    ...
    parser.add_argument("--from-step", type=float, default=0,
                        help="Start from this step number (0-17, use 6.5 for accession grounding)")
    ...
    steps = {
        ...
        16:  ("Readout Analysis", lambda: step16_readout_analysis(db)),
        17:  ("Generate Figures", lambda: step17_generate_figures(db)),
    }
