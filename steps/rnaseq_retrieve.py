...
from data_layer.geo_matrix_fetcher import (
    ...
    fetch_geo_supplementary_counts,
    parse_series_matrix,
...
def retrieve_geo_expression(db: PipelineDB, paper: dict) -> int:
    ...
    for acc, matrix_path in zip(own_data_accs, matrix_paths):
        ...
        if not gene_data:
            # Check platform: RNA-seq series matrices lack probe-to-gene
            # annotation, so fall back to supplementary count files
            gpl = None
            if matrix_path:
                from data_layer.geo_matrix_fetcher import _extract_platform_id
                gpl = _extract_platform_id(matrix_path)
            if not gpl:
                gpl = (acc.get("platform") or "").replace("GPL ", "GPL")
            if gpl and gpl in RNASEQ_GPLS:
                logger.info("[%s] %s is RNA-seq platform %s — trying supplementary files",
                            pmc_id, gse_id, gpl)
                gene_data = fetch_geo_supplementary_counts(gse_id, ALL_TARGET_GENES, GEO_MATRIX_DIR)
                from_suppl = bool(gene_data)
            if not gene_data:
                logger.info("[%s] No target genes found in %s", pmc_id, gse_id)
                continue

        source_label = f"{gse_id} supplementary" if from_suppl else f"{gse_id} series matrix"
        conf = 0.90 if from_suppl else 0.95
