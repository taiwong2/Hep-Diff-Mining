# Hepatocyte Differentiation Mining

[![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.20050025.svg)](https://doi.org/10.5281/zenodo.20050025)
...
git clone https://github.com/taiwong2/Hep-Diff-Mining.git
cd Hep-Diff-Mining
...
```bash
# Run the complete 18-step pipeline
...
| 16 | Readout Analysis | Rank expression readouts by consistency and dynamic range |
| 17 | Figure Generation | Generate all manuscript figures from database state |
...
```
run_pipeline.py                    — 18-step pipeline orchestrator
...
scripts/                           — Standalone utility workflows
  readout_analysis.py              — Step 16: expression readout ranking
  generate_figures_v2.py           — Step 17: manuscript figure generation
...
manuscript/                        — Paper manuscript and figures
  main.tex                         — Main manuscript (Nature format)
  supplementary.tex                — Supplementary information
  figures/                         — Generated figures (19 panels)
...
- **`rnaseq_metadata`** — RNA-seq technology and analysis pipeline details
- **`repository_metadata`** — GEO/SRA/ENA accession metadata and fetch status
- **`expression_values`** — Gene expression measurements (47-gene panel across protocols)
- **`geo_accessions`** — Discovered and validated GEO accession numbers
- **`geo_samples`** — Individual GEO samples with platform and condition metadata
- **`geo_sample_stage_mappings`** — Mapping of GEO samples to protocol differentiation stages
...
```bibtex
@software{wong2026hepdiff,
  author = {Wong, Tai and Mattis, Aras},
  title = {Hepatocyte Differentiation Mining: LLM-Based Protocol Extraction from Biomedical Literature},
  year = {2026},
  publisher = {Zenodo},
  doi = {10.5281/zenodo.20050025},
  url = {https://doi.org/10.5281/zenodo.20050025}
}
```
